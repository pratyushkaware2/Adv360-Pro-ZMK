...
# West Build (left)
west build -s zmk/app -p -d build/left -b adv360_left -S studio-rpc-usb-uart -- -DZMK_CONFIG="${PWD}/config" -DEXTRA_CONF_FILE="${PWD}/config/prj.conf" -DCONFIG_ZMK_STUDIO=y
...
if [ "${BUILD_RIGHT}" = true ]; then
    # West Build (right)
    west build -s zmk/app -p -d build/right -b adv360_right -- -DZMK_CONFIG="${PWD}/config" -DEXTRA_CONF_FILE="${PWD}/config/prj.conf"
